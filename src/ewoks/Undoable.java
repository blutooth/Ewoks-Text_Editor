...
public abstract class Undoable<T> {
    ...
    /**MemoryText is a TextStack that may be used by PasteScraps 
     *so that they may store information for future PasteScraps
     *MemoryText is a short-term store of memory and reset by undoable.perform(). */
    private TextStack memoryText = new TextStack();
	...
	/** An element of the undo history that can control memoryText */
    public  abstract class MemoryScrap extends Scrap {
        /** Push text onto memoryText  */
        public  void push(Text.Immutable txt) {memoryText.push(txt);}

        /** Reset memoryText */
        public  void reset() {memoryText.reset();}

        /** Pop text from emoryText */
        public Text.Immutable pop() { return memoryText.pop(); }
    ...
    public static <T extends Undoable<T>> AbstractAction<T>
        ...
        return new ReflectAction<>(cl, name, args);
    }
}
